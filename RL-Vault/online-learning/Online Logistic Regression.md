...
The running example Celli uses in the intro lecture (slides 14, 19, 22) to motivate why we need a *generic* [[Online Convex Optimization|OCO]] framework: unlike [[Online Ridge Regression|ridge]], logistic regression has **no closed-form solution**, so there's no analog of Sherman-Morrison to fall back on.
...
Celli's intro lecture (slide 14, 19, 22) sets up logistic regression as:
